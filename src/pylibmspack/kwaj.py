...
def _raise_for_err(err: int, context: str) -> None:
    ...
    if err == _ERR_OUTPUT_LIMIT:
        raise KwajError(f"{context} failed: output exceeds max_size")
...
class KwajFile:
    ...
    def read(self, *, max_size: int = _DEFAULT_MAX_SIZE) -> bytes:
        ...
        with tempfile.TemporaryDirectory(prefix="pylibmspack-") as tmp:
            ...
            if self._data is not None:
                err = _cab.kwaj_extract_bytes(self._data, out_path, max_size)
            else:
                err = _cab.kwaj_extract(self.path, out_path, max_size)
