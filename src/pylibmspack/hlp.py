...
def _check_not_winhelp_container(data: bytes) -> None:
    if data.startswith(_WINHELP_MAGIC):
        raise HlpFormatError(
            "full WinHelp .hlp containers are not supported by bundled libmspack 0.10.1"
        )
...
class HlpFile:
    ...
    The bundled libmspack 0.10.1 source does not implement the full WinHelp
    .hlp container parser. This class handles the MS Help-flavored LZSS stream
    used inside those files.
    """
    ...
    def read(self, *, max_size: int = 256 * 1024 * 1024) -> bytes:
        ...
        with tempfile.TemporaryDirectory(prefix="pylibmspack-") as tmp:
            ...
            if self._data is not None:
                err = _cab.hlp_decompress_bytes(self._data, out_path)
            else:
                err = _cab.hlp_decompress(self.path, out_path)
